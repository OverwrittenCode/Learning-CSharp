...
internal abstract class BaseScene
{
    ...
    /// <summary>
    ///     Begins the scene. This should lead up to the <see cref="Choices" /> shown afterward.
    /// </summary>
    ...
    public BaseScene? GetNextScene()
    {
        ...
        {
            Console.Write("> ");
            if (Console.ReadLine()?.ToUpper().Trim() is { Length: 1 } input && choiceLetters.IndexOf(input, StringComparison.Ordinal) is var index and >= 0)
            {
