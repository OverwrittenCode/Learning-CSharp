...
internal sealed class Player : HumanoidBase
{
    ...
    private static char GetDiffSymbol<T>(T diff) where T : struct, IComparable<T>
    {
        var diffValue = Convert.ToDouble(diff);
    ...
    /// </summary>
    public List<CollectableBase> Inventory { get; } = [];
    ...
    public int Lives
    {
        ...
        private set
        {
            ...
            if (diff != -1)
            {
                throw new ArgumentOutOfRangeException(nameof(value), "You may only reduce Lives by 1.");
            }
            ...
            if (_lives == 0)
            {
                ...
                {
                    var inventoryDisplay = String.Join(", ", Inventory.Select(item => item.Name));
    ...
    public Player(string name, string description, decimal money, ConsoleColor dialogueColour) : base(name, description, dialogueColour)
    {
    ...
    public T? GetItem<T>() where T : CollectableBase => Inventory.Find(collectable => collectable is T) as T;
    ...
    public bool GiveItem<T>(Player target) where T : CollectableBase
    {
        if (target == this)
        {
    ...
    /// <inheritdoc cref="Attack" path="/returns" />
    public bool InteractWithItem<T>(bool orThrow = false) where T : CollectableBase
    {
        var item = GetItem<T>();
    ...
    public bool GiveMoney(Player target, decimal amount)
    {
        ...
        var givenAmount = Math.Max(amount, Money);
    ...
    public bool StealMoney(Player target, decimal amount)
    {
        ...
        var stolenAmount = Math.Min(amount, target.Money);
    ...
    private void Inform(string text, string? total = null)
    {
        ...
        var message = $"[Info for {DisplayName}]: {text}";

        Action<string>? provider = total is null
            ? Utils.TypewriterEffect
            : message =>
            {
                var pipeColumn = Console.WindowWidth - 25;
                var messagePadding = Math.Max(0, pipeColumn - message.Length);
                var lineEnding = new string(' ', messagePadding) + $"| Total: {total}";
